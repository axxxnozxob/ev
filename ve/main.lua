...
local function safeGetDownedPosition(targetPlayer)
    ...
    local success, position = pcall(function()
        ...
        if ragdoll then
            local root = ragdoll:FindFirstChild("HumanoidRootPart") or ragdoll:FindFirstChild("Torso") or ragdoll:FindFirstChild("Head")
...
local AntiNextbotModule = (function()
            ...
            if hrp and not char:GetAttribute("Downed") then
                ...
                if isTooClose and nearestBot then
                    ...
                    if not farmsSuppressed then
                        if Toggles.AutoFarmMoney and Toggles.AutoFarmMoney.Value then prevMoney = true; Toggles.AutoFarmMoney:SetValue(false) end
                        if Toggles.AutoFarmTickets and Toggles.AutoFarmTickets.Value then prevTicket = true; Toggles.AutoFarmTickets:SetValue(false) end
                    ...
                    if evadeType == "Spawn" then
                        local spawns = workspace:FindFirstChild("Game") and workspace.Game:FindFirstChild("Map") and workspace.Game.Map:FindFirstChild("Parts") and workspace.Game.Map.Parts:FindFirstChild("Spawns")
                    ...
                    elseif evadeType == "Players" then
                        ...
                        for _, p in pairs(Players:GetPlayers()) do
                            if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") and not p.Character:GetAttribute("Downed") then
                                table.insert(plrs, p)
                            end
                        end
                        if #plrs > 0 then
                            hrp.CFrame = plrs[math.random(1, #plrs)].Character.HumanoidRootPart.CFrame + Vector3.new(0, 3, 0)
                        end
                ...
                elseif not isTooClose and farmsSuppressed then
                    ...
                    if prevMoney then Toggles.AutoFarmMoney:SetValue(true); prevMoney = false end
                    if prevTicket then Toggles.AutoFarmTickets:SetValue(true); prevTicket = false end
    ...
    return {
        Start = function() enabled = true; defenseTask = task.spawn(scanAndEvade); Success("Defense", "Anti-Nextbot Aktif!", 2) end,
        Stop = function() enabled = false; if defenseTask then task.cancel(defenseTask) end; Info("Defense", "Anti-Nextbot Mati", 2) end,
        SetRange = function(v) detectionRange = v end,
        SetType = function(v) evadeType = v end,
        SetEvadeDistance = function(v) evadeDistance = v end
    }
end)()
...
local UnifiedAutoFarm = (function()
            ...
            while currentMode ~= "None" do
                ...
                if hrp then
                    ...
                    if char:GetAttribute("Downed") then
                        pcall(function() changeModeEvent:FireServer(true) end)
                        task.wait(0.5)
                    ...
                    if currentMode == "Money" then
                        ...
                        for _, pl in ipairs(Players:GetPlayers()) do
                            -- Tambahan: Cek apakah target masih valid di server
                            if pl ~= player and safeIsPlayerDowned(pl) then
                                local pos = safeGetDownedPosition(pl)
                                if pos then
                                    local d = (hrp.Position - pos).Magnitude
                                    if d < minDist then target = pl; minDist = d end
                                end
                            end
                        end
                        ...
                        if target then
                            ...
                            -- 2. Tunggu sebentar agar server sinkron dengan posisi kita (Mencegah error UserId)
                            task.wait(0.1) 
                            
                            -- 3. Cek lagi, apakah selama kita TP tadi sudah ada yang menolong?
                            if safeIsPlayerDowned(target) then
                                pcall(function() 
                                    interactEvent:FireServer("Revive", true, target.Name) 
                                end)
                                
                                -- 4. BERTAHAN DI SINI SEBENTAR (Penting!)
                                -- Jangan langsung TP balik. Stay 0.2 detik agar UI Popup game tidak bingung mencari UserId kita.
                                task.wait(0.2) 
                            end
                            
                            waitTime = 0.1
                        else
                            -- KEMBALI KE SAFESPOT jika map bersih
    ...
    return {
        SetMode = function(m) currentMode = m; if m ~= "None" then startMasterLoop() end end,
...
local AntiAFKModule = (function()
    ...
    return {
        ...
        IsEnabled = function() return connection ~= nil end
    }
end)()
...
local EasyTrimpModule = (function()
        ...
        while bounceEnabled or momentumEnabled do
            ...
            if root and hum then
                ...
                if momentumEnabled then
                    ...
                    if pushForce then pushForce:Destroy() end
                    ...
                    if spdDisplay then spdDisplay.Text = tostring(math.floor(currentSpeed * 10) / 10) end
                end
            end
            task.wait()
        end
    ...
    local function checkTask()
        if (bounceEnabled or momentumEnabled) then
            ...
            if pushForce then pushForce:Destroy() pushForce = nil end
            currentSpeed = settings.baseSpeed
        end
    end

    return {
        ToggleBounce = function(state) bounceEnabled = state checkTask() end,
        ToggleMomentum = function(state) momentumEnabled = state checkTask() end,
        SetPower = function(v) settings.bouncePower = v end,
        SetDist = function(v) settings.groundDistance = v end,
        SetBase = function(v) settings.baseSpeed = v end,
        SetMax = function(v) settings.extraSpeed = v end,
        SetDrop = function(v) settings.floorDrop = v end,
        StopAll = function() bounceEnabled = false momentumEnabled = false checkTask() end,
        IsEnabled = function() return bounceEnabled or momentumEnabled end
    }
end)()
    ...
    local function enhanceReviveGrenade()
        local success, result = pcall(function()
            ...
            local function applyMods(rg)
                if not rg or not rg.Tasks or not rg.Tasks[1] then return end
                ...
                if task.Functions and task.Functions[1] and task.Functions[1].Activations and task.Functions[1].Activations[1] then
...
local AutoJumpModule = (function()
        ...
        for _, obj in pairs(getgc(true)) do
            if type(obj) == "table" and rawget(obj, "Friction") then
                ...
                if safeOriginal < 0 then
                    safeOriginal = 5 -- Default normal friction
                end
    ...
    local function applyBhopFriction()
        ...
        if isActive and bhopMode == "Acceleration" then
            -- Hanya scan memory JIKA tabel kosong (mencegah lag saat tekan spasi)
    ...
    local function loadBhop()
        -- FIX: Hapus pemanggilan findFrictionTables() di sini yang bikin macet!
        -- Biarkan applyBhopFriction() yang mengecek lewat cache.
        applyBhopFriction()
    ...
    characterConnection = player.CharacterAdded:Connect(function(character)
        ...
        -- Kosongkan cache saat respawn agar discan ulang sekali saja
        frictionTables = {}
...
local SuperLaunchModule = (function()
        ...
        inputBeganConn = uis.InputBegan:Connect(function(input, gameProcessed)
            ...
            if input.UserInputType == Enum.UserInputType.MouseButton1 and isVHeld then
                ...
                if hrp then
                    ...
                    hrp.AssemblyLinearVelocity = Vector3.new(0, 0, 0) -- Reset momentum
                    
                    if launchDirection == "Mouse" then
                        -- Melontarkan karakter ke arah kursor mouse
    ...
    return {
        ...
        Stop = stop,
        IsEnabled = function() return enabled end,
        SetHeight = function(val) launchHeight = val end,
        SetDistance = function(val) launchDistance = val end,
        SetDirection = function(val) launchDirection = val end
    }
end)()
...
local InfiniteSlideModule = (function()
    ...
    -- Penanda agar script tidak spam ubah angka setiap detik
    ...
    local function slideUpdate()
        ...
        if state == "Slide" then
            -- Mencegah animasi berhenti dengan mengubah attribute
            pcall(function()
            ...
            if not isCurrentlySliding then
                ...
                setSlideFriction(slideFrictionValue) -- Ubah licin SEKALI saja
            end
        ...
        elseif state == "EmotingSlide" then
            ...
            -- Kalau tidak lagi meluncur, kembalikan ke normal SEKALI saja
            if isCurrentlySliding then
                isCurrentlySliding = false
                setSlideFriction(5) -- 5 adalah gesekan normal Evade
            end
        end
    end
    ...
    return {
        ...
        SetSlideSpeed = function(speed)
            ...
            if num then
                ...
                -- Update instan kalau sedang meluncur saat angka diganti
...
-- ========================================================================== --
--                  MODERN FLOATING FPS, MS & TIMER MODULE                    --
-- ========================================================================== --
...
local FPSTimerDisplayModule = (function()
    ...
    -- Helper function untuk membuat GUI bisa di-drag
    ...
    local function start()
        ...
        task.spawn(function()
            ...
            if coreGui:FindFirstChild("RZ_FloatingStats") then
                coreGui.RZ_FloatingStats:Destroy()
            end

            -- Create ScreenGui
            local screenGui = Instance.new("ScreenGui")
            screenGui.Name = "RZ_FloatingStats"
            screenGui.ResetOnSpawn = false
            screenGui.Parent = coreGui
            displayGui = screenGui

            -- Main Frame (The Container)
            ...
            mainFrame.Position = UDim2.new(0.5, -100, 0, 50) -- Default tengah atas
            mainFrame.Size = UDim2.new(0, 250, 0, 28)
            mainFrame.Active = true
            mainFrame.Draggable = false -- Kita pakai custom drag helper di bawah
            ...
            infoLabel.Font = Enum.Font.Code -- Pakai font monospaced agar angka tidak goyang
            infoLabel.TextSize = 14
            infoLabel.Text = "rzprivate | 0 fps | 0 ms | 0:00"
            ...
            updateConnection = RunService.Heartbeat:Connect(function()
                ...
                -- Update Label
                infoLabel.Text = string.format("rzprivate | %d fps | %d ms | %s", currentFPS, ping, timerText)
            end)
    ...
    local function stop()
        ...
        if displayGui then displayGui:Destroy() end
...
local ArrayListModule = (function()
    ...
    local function updateList()
        if not enabled or not listFrame then return end
        ...
        if Toggles then
            for toggleIdx, toggle in pairs(Toggles) do
                ...
                if toggle.Value and toggle.Text and toggleIdx ~= "KeybindMenuOpen" and toggleIdx ~= "ShowCustomCursor" and toggleIdx ~= "EnableNotifications" and toggleIdx ~= "ShowArrayList" then
...
local LagSwitchModule = (function()
    ...
    local function executeLag()
        if not enabled then return end -- Harus ON dulu togglenya
    ...
    return {
        ...
        SetMode = function(mode) lagMode = mode end,
        SetDelay = function(val) lagDelay = tonumber(val) or 0.1 end,
        SetIntensity = function(val) lagIntensity = tonumber(val) or 1000000 end,
        SetDemonHeight = function(val) demonHeight = tonumber(val) or 10 end,
        SetDemonSpeed = function(val) demonSpeed = tonumber(val) or 80 end,
        IsEnabled = function() return enabled end
    }
end)()
...
local AutoBuyModule = (function()
    ...
    local function getCategories()
        ...
        table.sort(cats)
        if #cats == 0 then table.insert(cats, "None") end
        return cats
    end
    ...
    local function purchase(itemName, amount)
        ...
        task.spawn(function()
            ...
            for i = 1, amount do
                ...
                if success and result == true then bought = bought + 1 end
                task.wait(0.2)
            end
    ...
    return {
        ...
        Start = startAutoBuy,
        Stop = stopAutoBuy,
        SetItem = function(itemName) autoBuyItemName = itemName end,
        SetDelay = function(delay) buyDelay = delay end,
        IsEnabled = function() return enabled end,
...
local EmoteChangerModule = (function()
    ...
    local function fireSelect(slot)
        ...
        if not selectEmotes[slot] or selectEmotes[slot] == "" or selectEmotes[slot] == "None" then return end
    ...
    local function init()
        ...
        if not (PassCharacterInfo and EmoteRemote) then return end

        PassCharacterInfo.OnClientEvent:Connect(function(...)
            if not getgenv().pendingSlot then return end
    ...
    local function isValidEmote(emoteName)
        if emoteName == "" or emoteName == "None" then return false, "" end
    ...
    local function applyMappings()
        ...
        if not hasAnyEmote then return false, "Silakan isi emote terlebih dahulu!" end
    ...
    return {
        Init = init,
        SetCurrent = function(slot, val) currentEmotes[slot] = val:gsub("%s+", "") end,
        SetSelect = function(slot, val) selectEmotes[slot] = val:gsub("%s+", "") end,
        Apply = applyMappings,
        Reset = function()
            for i = 1, 5 do
                currentEmotes[i] = ""
...
-- ========================================================================== --
    ...
    local Window = Library:CreateWindow({
        ...
        AutoShow = true, -- 🔴 KEMBALIKAN INI JADI TRUE AGAR MUNCUL
    })
...
local InfoRight = Tabs.Info:AddRightGroupbox("Telegram", "users")
...
-- DISCORD & COMMUNITY
InfoRight:AddLabel("Join Telegram Community", true)

InfoRight:AddInput("Telegram", {
    Default = "t.me/rzprvt",
    Numeric = false,
    Finished = true,
    Text = "Telegram Server",
...
InfoRight:AddButton({
    ...
    Func = function()
        -- FIX: Ubah telegramLink menjadi Telegram
        ...
        if success then
            Success("Telegram", "Link copied to clipboard!", 2)
        else
            Error("Telegram", "Clipboard not supported by your executor", 2)
        end
    end
})
...
InfoRight:AddLabel("• Join Telegram for support", true)
...
local AntiBotGroup = Tabs.Combat:AddLeftGroupbox("Anti-Nextbot Defense", "shield")
...
CombatLeft:AddToggle("AutoSelfRevive", {
    ...
    Default = false,
}):AddKeyPicker("AutoSelfReviveKey", { Default = "", SyncToggleState = true, Mode = "Toggle", Text = "Auto Self Revive" })
...
CombatLeft:AddToggle("InstantRevive", {
    ...
    Default = false,
}):AddKeyPicker("InstantReviveKey", { Default = "", SyncToggleState = true, Mode = "Toggle", Text = "Instant Revive" })
...
local function updateModuleStatus()
    ...
    if isLoaded then
        ...
        if hasMap then
            if TeleportModule.GetMapSpot(mapName, "Far") then farStatus = "✅" end
            if TeleportModule.GetMapSpot(mapName, "Sky") then skyStatus = "✅" end
        end
        
        -- Susun teks agar sangat rapi
        statusText = string.format(
            "Current Map: %s\nMap Status: %s\nSpots: 📍 Far [%s]  |  ☁️ Sky [%s]\nDatabase: %d maps (%s)",
            mapName, mapStatus, farStatus, skyStatus, mapCount, lastUpdate
        )
    else
...
local EmoteChangerGroup = Tabs.Emotes:AddLeftGroupbox("Emote Replacer", "smile")

EmoteChangerGroup:AddLabel("Ganti emote gratisan jadi yang mahal! (5 Slot)", true)
EmoteChangerGroup:AddDivider()

local emoteList = {
    "None", "APose", "Addendum", "Aerobics", "AngelicWings", "AprilShower", "AuspiciousWell", "AvariceLounge", 
    "BeachChairLounge", "Beg", "Bhop", "BobointheBox", "BoldMarch", "Boneless", "BoogieDown", 
    "Breakdown", "BringItAround", "Broom", "Bumblebee", "BumperKart", "Caffeinated", "California", 
    "CampfireDoze", "Caramelldansen", "Carlton", "CasualSurfing", "CatDance", "CatParty", "Catdown", 
    "Catjam", "ChristmasBoogie", "Clap", "ClassicDance", "ClassicJeep", "ClassicStride", "ClubDance", 
    "CompanyMan", "Conga", "CozyChair", "Crabby", "CrouchDance", "CuerdasDelAlma", "CyberBroom", 
    "DeadBoneStride", "Distraction", "DogParty", "DreamyCloud", "DuckyMarch", "DynastyDrumming", 
    "Epicaricacy", "Facepalm", "FastFoodDelight", "Fazbore", "FireworkBlast", "FlashingLights", 
    "Flexing", "FlyingFish", "FlyingSleigh", "Freestyle", "FreshFlop", "FrightFunk", "Frolic", 
    "FrostDrake", "Gangnam", "GhastlyGrimoire", "GhoulishGalleon", "Gmod", "GoldRitual", "GoofyStride", 
    "GraveRider", "Griddy", "Gyrating", "HarpRecital", "HeadlessBaller", "HeadlessHorseman", 
    "Heaventaker", "Hired", "HoveringCrystal", "IceFishing", "Infectious", "IrishJig", "Kickback", 
    "LDance", "LemonadeStand", "LineDance", "LittleJiggy", "LunarParty", "M3GANDance", "Macarena", 
    "ManyFans", "MaracaTime", "MarchShowcase", "Marching", "MariachiBand", "Mashle", "Moonwalk", 
    "Nizmoo", "Nostalgia", "Nutcracker", "OiiaOiia", "PBJT", "ParkerPride", "PawsClaws", "PonPon", 
    "PoolTime", "Popipopi", "PotionMash", "PumpItUp", "RainingTacos", "Rambunctious", "Reanimated", 
    "Robot", "RobotM3GAN", "RockefellerStreet", "Rocket", "RockinStride", "RockingHorse", "RowBoat", 
    "RudolphMount", "RushinAround", "RussianDance", "SantaMech", "ScorchedEarth", "SeeTinh", 
    "SerenePerch", "SeriousMarch", "ShubaDance", "Sit", "SkateboardStroll", "SkiSpree", "SledDrifting", 
    "Sleep", "Sleepybara", "Smile", "Smug", "SnowAngel", "SnowmanConstruction", "SnowmobileCruise", 
    "SolarBike", "SolarConqueror", "SolarSlayer", "SpiritedAway", "SpookyTime", "StarPower", "Stride", 
    "SummerDays", "SwagWalk", "TPose", "Tank", "TexasStyle", "Thriller", "TouchGrass", "ToyTrainRide", 
    "TurkeyJockey", "TurtleHobble", "Twist", "ValentineComputer", "WerewolfHowl", "Wess", "WindUpPose", 
    "WindupDance", "WinterMelody", "WinterRide", "Writhing", "Xylobone", "ZenSerenity", "ZombieStride"
}
...
for i = 1, 5 do
    ...
    if i < 5 then EmoteChangerGroup:AddDivider() end
end

local EmoteControlsGroup = Tabs.Emotes:AddRightGroupbox("Controls", "settings")
...
EmoteControlsGroup:AddDivider()
EmoteControlsGroup:AddLabel("⚠️ Catatan:\nGambar icon emote di game TIDAK AKAN BERUBAH. Ini wajar dan membuktikan bypass aman!", true)

-- ========================================================================== --
--                             AUTO FARM TAB (V3.2)                           --
-- ========================================================================== --

local AutoFarmLeft = Tabs.AutoFarm:AddLeftGroupbox("Main Farm", "coins")
local AutoFarmRight = Tabs.AutoFarm:AddRightGroupbox("Idle Mode", "moon")
...
Toggles.AutoFarmMoney:OnChanged(function()
    if Toggles.AutoFarmMoney.Value then
        ...
        if UnifiedAutoFarm.GetMode() == "Money" then UnifiedAutoFarm.SetMode("None") end
    end
end)
...
Toggles.AutoFarmTickets:OnChanged(function()
    if Toggles.AutoFarmTickets.Value then
        ...
        if UnifiedAutoFarm.GetMode() == "Ticket" then UnifiedAutoFarm.SetMode("None") end
    end
end)
...
Toggles.AFKFarm:OnChanged(function()
    if Toggles.AFKFarm.Value then
        ...
        if UnifiedAutoFarm.GetMode() == "AFK" then UnifiedAutoFarm.SetMode("None") end
    end
end)
...
-- ========================================================================== --
local TrimpGroup = Tabs.Movement:AddRightGroupbox("Trimping System", "zap")
...
TrimpGroup:AddSlider("BouncePower", {
    Text = "Bounce Power",
    Default = 100, Min = 50, Max = 300, Rounding = 0,
    Callback = function(v) EasyTrimpModule.SetPower(v) end
})

TrimpGroup:AddSlider("GroundDistance", {
    Text = "Ground Check Distance",
    Default = 6, Min = 2, Max = 15, Rounding = 1,
    Callback = function(v) EasyTrimpModule.SetDist(v) end
})
...
TrimpGroup:AddSlider("MomentumBase", {
    ...
    Default = 50, Min = 10, Max = 150, Rounding = 0,
    Callback = function(v) EasyTrimpModule.SetBase(v) end
})

TrimpGroup:AddSlider("MomentumMax", {
    Text = "Max Air Boost Speed",
    Default = 100, Min = 0, Max = 500, Rounding = 0,
    Callback = function(v) EasyTrimpModule.SetMax(v) end
})

TrimpGroup:AddSlider("LandingLoss", {
    Text = "Landing Speed Loss",
    Default = 0, Min = 0, Max = 50, Rounding = 0,
    Callback = function(v) EasyTrimpModule.SetDrop(v) end
})
...
MovementRight2:AddDropdown("LaunchDirection", {
    Values = { "Maju (Ke Depan)", "Mundur (Ke Belakang)", "Kiri (Ke Kiri)", "Kanan (Ke Kanan)", "Tetap (Lurus Ke Atas)", "Ke Arah Kursor (Mouse)" },
...
VisualLeft:AddToggle("RemoveBarriers", {
    ...
    Default = false,
}):AddKeyPicker("RemoveBarriersKey", { Default = "", SyncToggleState = true, Mode = "Toggle", Text = "Remove Barriers" })
...
VisualLeft:AddToggle("BarriersVisible", {
    ...
    Default = false,
}):AddKeyPicker("BarriersVisibleKey", { Default = "", SyncToggleState = true, Mode = "Toggle", Text = "Barriers Visible" })
...
ServerRight2:AddDropdown("LagSwitchMode", {
    ...
    Callback = function(Value) LagSwitchModule.SetMode(Value) end
})
...
ServerRight2:AddInput("LagDelay", {
    ...
    Callback = function(Value) LagSwitchModule.SetDelay(Value) end
})
...
ServerRight2:AddInput("LagIntensity", {
    ...
    Numeric = true,
    Text = "Lag Intensity",
    Callback = function(Value) LagSwitchModule.SetIntensity(Value) end
})
...
ServerRight2:AddInput("DemonHeight", {
    ...
    Callback = function(Value) LagSwitchModule.SetDemonHeight(Value) end
})
...
ServerRight2:AddInput("DemonSpeed", {
    ...
    Callback = function(Value) LagSwitchModule.SetDemonSpeed(Value) end
})
...
local ShopLeft = Tabs.AutoBuy:AddLeftGroupbox("Manual Buy", "shopping-bag")
local ShopRight = Tabs.AutoBuy:AddRightGroupbox("Auto Buy System", "refresh-cw")
...
ShopLeft:AddButton({
    Text = "Beli 1x",
    Func = function() AutoBuyModule.Purchase(selectedManualItem, 1) end
})

ShopLeft:AddButton({
    Text = "Beli 5x",
    Func = function() AutoBuyModule.Purchase(selectedManualItem, 5) end
})

ShopLeft:AddButton({
    Text = "Beli 10x",
    Func = function() AutoBuyModule.Purchase(selectedManualItem, 10) end
})
...
ShopLeft:AddButton({
    ...
    Func = function() AutoBuyModule.PrintScannedItems() end
})
...
Library:OnUnload(function()
    ...
    if AntiNextbotModule then
        AntiNextbotModule.Stop() -- Penting! Biar gak auto TP lagi
    end
    
    if UnifiedAutoFarm then
        UnifiedAutoFarm.SetMode("None") -- Menghentikan semua loop farm
    end

    if AntiAFKModule then AntiAFKModule.Stop() end

    -- === MODUL COMBAT ===
    if AutoSelfReviveModule.IsEnabled() then AutoSelfReviveModule.Stop() end
    if InstantReviveModule.IsEnabled() then InstantReviveModule.Stop() end
    if AutoWhistleModule and AutoWhistleModule.IsEnabled() then AutoWhistleModule.Stop() end
    ...
    if AutoJumpModule and AutoJumpModule.IsEnabled() then AutoJumpModule.Stop() end
    ...
    -- === VISUAL & ESP ===
    if ESP_System and ESP_System.Running then ESP_System:Stop() end
    ...
    -- === UI EXTRAS ===
    if FPSTimerDisplayModule then pcall(function() FPSTimerDisplayModule.Cleanup() end) end
    if ArrayListModule then pcall(function() ArrayListModule.Cleanup() end) end
    if EmoteChangerModule then EmoteChangerModule.Reset() end
    
    -- === SERVER & GLOBAL ===
    if LagSwitchModule then LagSwitchModule.SetEnabled(false) end
    if UnlockLeaderboardModule then pcall(function() UnlockLeaderboardModule.Destroy() end) end
    
    if _G.AutoPlaceConnection then pcall(function() _G.AutoPlaceConnection:Disconnect() end) _G.AutoPlaceConnection = nil end
    if _G.MainCharacterConnection then pcall(function() _G.MainCharacterConnection:Disconnect() end) _G.MainCharacterConnection = nil end
